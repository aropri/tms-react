...
const styles = (theme) => ({
  ...
  const namelist = ['Anand', 'Wilson', 'Surya'];
  const truckno = ['111', '112', '113'];
    ...
    <Autocomplete
      ...
      options={truckno}
      fullWidth
      renderInput={(params) => <TextField {...params} label="Truck No" 
      variant="outlined" 
      color="primary" focused
      sx={{
        input: {
...
/>
         ...
         <Button variant="outlined" className={classes.loginBtn} onClick={() => {navigate("/SubconDo1")}} 
         sx={{
            background: "#372a3d",
            borderRadius: "5px",
            width: "20%",
         ...
         <Typography variant="caption" >
           <Link  to="/Menu"  style={{ color: "#d2c2db" }}> Search by InvoiceNo?</Link>
         </Typography>
       </div>
       </div>
    )
  }
  export default(withStyles(styles)(SubconDo));